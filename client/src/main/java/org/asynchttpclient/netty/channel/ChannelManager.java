...
public class ChannelManager {
    ...
    wsBootstrap.handler(new ChannelInitializer<Channel>() {
      ...
      protected void initChannel(Channel ch) {
        ...
        if (config.isEnableWebSocketCompression()) {
          pipeline.addBefore(AHC_WS_HANDLER, WS_COMPRESSOR_HANDLER, MyWebSocketClientCompressionHandler.INSTANCE);
        }
  ...
  public void upgradePipelineForWebSockets(ChannelPipeline pipeline) {
    ...
    pipeline.addAfter(WS_ENCODER_HANDLER, WS_DECODER_HANDLER, new WebSocket08FrameDecoder(false, true, config.getWebSocketMaxFrameSize()));
